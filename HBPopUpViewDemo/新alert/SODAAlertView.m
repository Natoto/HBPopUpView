...
@implementation SODATitleConfiguration
@end

@implementation SODAMessageConfiguration
@end

@interface SODAPopUpViewBtnModel : NSObject
@property (nonatomic, strong) UIButton *button;
@property (nonatomic, assign) SODAAlertBtnStyle btnStyle;
@property (nonatomic, copy) buttonAction actionHandler;
@end

@implementation SODAPopUpViewBtnModel
@end

@interface SODAAlertView () <UIGestureRecognizerDelegate>
@property (nonatomic, weak) UIView *contentView;
@property (nonatomic, strong) SODATitleConfiguration *title_Configuration;
@property (nonatomic, strong) SODAMessageConfiguration *msg_configuration;

@property (nonatomic, strong) NSMutableArray<UITextField *> *textFields;
@property (nonatomic, strong) NSMutableArray<SODAPopUpViewBtnModel *> *buttonModels;
@property (nonatomic, assign) SODAAlertViewStyle popUpViewStyle;
@end
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message {
    return [self initWithSODATitleConfiguration:^(SODATitleConfiguration *titleConf) {
        titleConf.text = title;
    } SODAMessageConfiguration:^(SODAMessageConfiguration *msgConf) {
...
- (instancetype) initWithTitle:(NSString *)title message:(NSString *)message cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSArray <NSString *>*)otherButtonTitles actionWithIndex:(void(^)(NSInteger index))action {
    SODAAlertView *popUpView = [self initWithSODATitleConfiguration:^(SODATitleConfiguration *titleConf) {
        titleConf.text = title;
    } SODAMessageConfiguration:^(SODAMessageConfiguration *msgConf) {
        msgConf.text = message;
    }];
    ...
    if (otherButtonTitles && otherButtonTitles.count) {
        for (int i = 0; i < otherButtonTitles.count; i ++) {
            ...
            if (btnTitle && btnTitle.length) {
                [popUpView addBtnWithTitle:btnTitle type:SODAAlertBtnStyleDefault handler:^{
...
-(instancetype) initWithSODATitleConfiguration:(void (^)(SODATitleConfiguration *configuration))SODATitleConfiguration SODAMessageConfiguration:(void (^)(SODAMessageConfiguration *configuration))msgConfiguration {
    self = [self init];
    if (self) {
        if (SODATitleConfiguration) {
            SODATitleConfiguration(_title_Configuration);
...
-(void)addBtnWithTitle:(NSString *)title type:(SODAAlertBtnStyle)style handler:(buttonAction)handler {
    ...
    SODAPopUpViewBtnModel *btnModel = [SODAPopUpViewBtnModel new];
...
- (void)_layoutTextFields {
    ...
    if (_popUpViewStyle == SODAAlertViewStyleAlert) {
...
- (void)_layoutButtons {
    ...
    }else if (_popUpViewStyle == SODAAlertViewStyleActionSheet) {
...
- (void) layoutBtnsForAlert {
    if (_buttonModels.count == 2) {
        ...
        for (int i = 0; i < _buttonModels.count; i ++) {
            SODAPopUpViewBtnModel *btnModel = _buttonModels[i];
        ...
        for (int i = 0; i < _buttonModels.count; i ++) {
            SODAPopUpViewBtnModel *btnModel = _buttonModels[i];
            if (btnModel.btnStyle == SODAAlertBtnStyleCancel) {
                offset --;
                continue;
            }
            nonCancelBtnCount ++;
...
- (void) layoutBtnsForActionSheet {
    ...
    for (int i = 0; i < _buttonModels.count; i ++) {
        ...
        if (btnModel.btnStyle == SODAAlertBtnStyleCancel) {
            if (_hasAddCancelBtnForOnce) {
                NSAssert(!_hasAddCancelBtnForOnce, @"SODAAlertView：在ActionSheet样式时，请不要添加两个以上的取消按钮");
                return;
            }else {
    ...
    //添加取消按钮
    if (_hasAddCancelBtnForOnce) {
        SODAPopUpViewBtnModel *cancelBtnModel = _buttonModels[_cancelBtnIndex];
...
- (UIColor *)colorWithBtnStyle:(SODAAlertBtnStyle)style {
    if (style == SODAAlertBtnStyleDefault) {
        return _btnStyleDefaultTextColor;
    }else if (style == SODAAlertBtnStyleCancel) {
        return _btnStyleCancelTextColor;
    }else if (style == SODAAlertBtnStyleDestructive) {
...
-(void)showInView:(UIView *)view preferredStyle:(SODAAlertViewStyle)preferredStyle {
    ...
    if (preferredStyle == SODAAlertViewStyleAlert) {
        _contentWidth = 265;
    ...
    }else if (preferredStyle == SODAAlertViewStyleActionSheet) {
        ...
        [UIView animateWithDuration:0.6 delay:0.0 usingSpringWithDamping:0.85 initialSpringVelocity:0.5
                            options:UIViewAnimationOptionCurveEaseIn animations:^{
                                ...
                                rect.origin.x = 0;
                                CGFloat offset = _hasAddCancelBtnForOnce ? _buttonHeight : 0;
...
- (void)btnAction:(UIButton *)sender {
    ...
    SODAPopUpViewBtnModel *model = _buttonModels[index];
